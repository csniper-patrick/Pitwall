# publisher/pitlane.py
"""
Connects to the F1 live timing websocket, processes pit lane data, and publishes
it to a Redis channel.

This script specifically subscribes to the 'PitLaneTimeCollection', 'PitStop', and
'PitStopSeries' data streams.
"""
                    ...
                    json.dumps(
                        {
                            "H": "Streaming",
                            "M": "Subscribe",
                            "A": [["PitLaneTimeCollection", "PitStop", "PitStopSeries"]],
                            "I": 1,
                        }
                    )
                )
                        ...
                        for msg in messages["M"]:
                            if msg["H"] == "Streaming":
                                ...
                                if channel == "PitLaneTimeCollection":
                                    # Handle deletions in the PitTimes dictionary.
                                    if deleted := delta["PitTimes"].pop("_deleted", None):
                                        for key in deleted:
                                            if reference and "PitTimes" in reference:
