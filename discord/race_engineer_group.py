...
async def get_active_driver():
    ...
    if sessionInfo["Type"] in ["Race", "Sprint"]:
        ...
        active_line = dict(
            filter(
                lambda item: item[1]["Retired"] == False
                and item[1]["ShowPosition"] == True,
        ...
        active_line = dict(
            filter(
                lambda item: item[1]["KnockedOut"] == False
                and item[1]["ShowPosition"] == True,
                timingDataF1["Lines"].items(),
            )
        )
...
class RaceEngineerGroup(app_commands.Group):
    ...
    async def tyres(self, interaction: discord.Interaction):
        ...
        for compound in compounds:
            embed = discord.Embed(
                title=compound,
                color=(
                    msgStyle["compoundColor"][compound]
                    if compound in msgStyle["compoundColor"]
                    else None
                ),
            )
    ...
    async def position(self, interaction: discord.Interaction):
        ...
        driver_style = {
            key: {
                ...
                "linestyle": ["solid", "dashed"][
                    idx % 2
                ],  # Alternate line styles for clarity
            }
        ...
        if cached_bytes:
            ...
            xvals = (
                []
            )  # Stores the last lap number for each driver, used to place labels correctly.
            for drv, info in sorted(
                ...
                xvals.append(
                    max(lap_no) if lap_no else 0
                )  # Store the last lap for the label.
