...
try:
    ...
    log.warning(
        f"Could not enable FastF1 cache (this is optional but recommended): {e}"
    )
...
if __name__ == "__main__":
    ...
    if DISCORD_BOT_TOKEN is None:
        ...
        try:
            ...
            log.critical(
                "FATAL ERROR: Improper token passed. Check the DISCORD_BOT_TOKEN value."
            )
        except Exception as e:
            log.critical(
                f"FATAL ERROR: An unexpected error occurred during bot execution: {e}",
                exc_info=True,
            )
